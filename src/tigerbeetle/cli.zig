...
pub fn parse_args(allocator: std.mem.Allocator) !Command {
    ...
    const raw_command = args.next() orelse fatal("no command provided, expected 'start', 'format', or 'version'", .{});
...
fn parse_size_to_count(comptime T: type, string_opt: ?[]const u8, comptime default: u32) u32 {
    ...
    if (string_opt) |string| {
        ...
        const count = math.cast(u32, count_u64) orelse fatal("size value is too large: '{s}'", .{string});
